...
impl<AppSt, Err, E>
    MqttServer<
        ...
        ControlFactory<AppSt, control::DefaultControlService<Session<AppSt>, E, Encoded>, E>,
        InFlightService,
    >
where
    AppSt: 'static,
        ...
        > + 'static,
{
    ...
    where
        ...
        E: From<Srv::Error>,
        Srv:
            ServiceFactory<(), ProtocolMessage, Session<AppSt>, Res = ProtocolMessageAck> + 'static,
        Srv::InitError: Error,
    {
        MqttServer {
            handshake: self.handshake,
    ...
    where
        Srv: ServiceFactory<(), Control<E>, Session<AppSt>, Res = Option<Encoded>> + 'static,
        Err: From<Srv::Error>,
...
impl<AppSt, H> Service<(), IoBoxed> for HandshakeService<AppSt, H>
where
    H: Service<(), Handshake, Res = HandshakeAck<AppSt>> + 'static,
{
...
mod tests {
    ...
    fn test_debug() {
        ...
        let _ = server.publish(async |p: Publish| Ok::<_, TestError>(p.ack()));
    }
}
