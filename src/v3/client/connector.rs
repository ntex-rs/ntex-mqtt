...
{
    type Res = Client;
    type Error = Error<ClientError<codec::ConnectAck>>;
    ...
    IoBoxed: From<S::Res>,
{
    async fn connect_inner(
    ...
    ) -> Result<Client, Error<ClientError<codec::ConnectAck>>> {
        let io: IoBoxed = ctx
        ...
        match packet {
            ...
            Decoded::Packet(p, _) => Err(ClientError::from(ProtocolError::unexpected_packet(
                p.packet_type(),
